...
This server can be tested locally with `npx expo serve` (available in Expo SDK 52 and later), visit the URL in a web browser or create a native build with the `origin` set to the local server URL. You can deploy the server for production using [EAS Hosting](/eas/hosting/get-started) or another third-party service.
...
> This is an alpha feature starting in SDK 52 and later. The process will be more automated and have better support in future versions.
