...
| `collapseId` | Android and iOS | `string` | An identifier for collapsing notifications. On Android, this only coalesces messages in transit (if the device is offline, only the latest one with a given `collapseId` is delivered) and maps to the FCM [`collapse_key`](https://firebase.google.com/docs/cloud-messaging/concept-options#collapsible_and_non-collapsible_messages). To also replace already-displayed notifications on Android, use `tag`. On iOS, this both coalesces messages in transit and replaces already-displayed notifications on the device and maps to [`apns-collapse-id`](https://developer.apple.com/documentation/usernotifications/generating-a-remote-notification#Send-a-POST-request-to-APNs). |
| `tag` | Android Only | `string` | An identifier for replacing notifications already displayed on the device. If the device is already showing a notification with the same `tag`, the new notification replaces it. This is separate from `collapseId`, which coalesces messages _in transit_, while `tag` replaces _already-displayed_ notifications. Maps to the FCM [`notification.tag`](https://firebase.google.com/docs/reference/fcm/rest/v1/projects.messages#AndroidNotification) field. |
| `mutableContent` | iOS Only | `boolean` | Specifies whether this notification can be [intercepted by the client app](https://developer.apple.com/documentation/usernotifications/modifying-content-in-newly-delivered-notifications?language=objc). Defaults to `false`. |
